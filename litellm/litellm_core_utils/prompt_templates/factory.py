...
def anthropic_messages_pt_xml(messages: list):
    ...
    if not new_messages or new_messages[0]["role"] != "user":
        if litellm.modify_params:
            new_messages.insert(0, {"role": "user", "content": [{"type": "text", "text": _DEFAULT_EMPTY_CONTENT_MESSAGE}]})
        else:
            raise Exception(
...
def _sanitize_empty_text_content(
    ...
    if isinstance(content, list):
        ...
        if len(new_blocks) != len(content):
            ...
            verbose_logger.debug(
                "_sanitize_empty_text_content: Removed empty text block(s) in %s message", message.get("role")
            )
